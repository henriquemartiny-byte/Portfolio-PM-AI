...
Importante: Retorne apenas o texto da história condensada direta, sem títulos, sem formatação markdown complexa ou tags.`;
    ...
    if (!apiKey) {
      console.warn("Warning: process.env.GEMINI_API_KEY is undefined in generateVinylStory.");
    }
    let ai;
    try {
...
// 7. Inicialização do Cliente Gemini (realizada de forma segura dentro das funções de execução)

export async function POST(req: NextRequest) {
  try {
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) {
      console.warn("Warning: process.env.GEMINI_API_KEY is undefined in POST route.");
...
- Retorne obrigatoriamente um objeto JSON com o formato:
{
  ...
  "artist": "Nome do artista para adicionar ou null"
    ...
    try {
      ...
      console.error("Gemini Execution Error:", error);
      return NextResponse.json(
        { error: "Erro de inicialização do cliente Gemini." },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      console.error("Gemini Execution Error:", error);
      return NextResponse.json(
        { error: "Erro ao executar chamada à API do Gemini." },
        { status: 500 }
      );
    }
